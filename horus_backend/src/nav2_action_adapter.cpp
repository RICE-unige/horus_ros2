  ...
  const std::string & status_topic)
{
  ...
  RCLCPP_INFO(node_->get_logger(), "Nav2 Action Adapter registered for robot: %s (%s)",
      robot_name.c_str(), robot_id.c_str());
}
...
void Nav2ActionAdapter::unregister_robot(const std::string & robot_id)
{
  ...
  if (it != adapters_.end()) {
    adapters_.erase(it);
    RCLCPP_INFO(node_->get_logger(), "Nav2 Action Adapter unregistered for robot: %s",
        robot_id.c_str());
  }
}
  ...
  const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  if (!adapter->action_client->wait_for_action_server(std::chrono::seconds(2))) {
    RCLCPP_ERROR(node_->get_logger(), "Action server not available after waiting: %s",
  ...
  const std_msgs::msg::String::SharedPtr msg)
{
  ...
  RCLCPP_INFO(node_->get_logger(), "Received cancel request for robot: %s",
      adapter->robot_id.c_str());
  ...
  if (adapter->active_goal_handle) {
    ...
    RCLCPP_INFO(node_->get_logger(), "Sent cancel request to Nav2 for robot: %s",
        adapter->robot_id.c_str());
  } else {
    RCLCPP_WARN(node_->get_logger(), "No active goal to cancel for robot: %s",
        adapter->robot_id.c_str());
...
{
  if (!goal_handle) {
    RCLCPP_ERROR(node_->get_logger(), "Goal was rejected by server for robot: %s",
        adapter->robot_id.c_str());
    publish_status(adapter, "goal_failed");
    return;
  }
  ...
  RCLCPP_INFO(node_->get_logger(), "Goal accepted by server, waiting for result for robot: %s",
      adapter->robot_id.c_str());
}
  ...
  const GoalHandleNav2::WrappedResult & result)
{
  ...
  switch (result.code) {
    ...
    case rclcpp_action::ResultCode::ABORTED:
      ...
      RCLCPP_ERROR(node_->get_logger(), "Goal was aborted for robot: %s",
        adapter->robot_id.c_str());
      break;
    case rclcpp_action::ResultCode::CANCELED:
      status_str = "goal_cancelled";  // Matches Unity's expected string
      RCLCPP_INFO(node_->get_logger(), "Goal was canceled for robot: %s",
        adapter->robot_id.c_str());
      break;
    default:
      status_str = "goal_unknown";
      RCLCPP_ERROR(node_->get_logger(), "Unknown result code for robot: %s",
        adapter->robot_id.c_str());
      break;
  }
...
void Nav2ActionAdapter::publish_status(
  std::shared_ptr<RobotAdapter> adapter,
