...
> [!NOTE]
> This roadmap is scoped to ROS 2 infrastructure ownership (bridge/runtime reliability, observability, and reproducibility), not Unity UX or SDK dashboard behavior. Current baseline aligns with the now-merged HorusLink reliable dual-lane transport and retained-state replay, marker-only mesh transport stability, remote-render RGB-D auxiliary delivery, explicit flat single-robot backend-topic compatibility, and bridge-global lease behavior across shared/private app workspace modes.
...
| WebRTC Media Reliability | :large_orange_diamond: In progress | Negotiation mismatch fixes, stall telemetry, paced keyframe recovery, signaling retention across DDS discovery, and corrected H.264 access-unit packetization are integrated. Remote-render sessions can also carry auxiliary depth and camera-pose payloads. | Resolve the remaining remote-render WebRTC reliability gap, then harden topology and scale behavior under high robot and stream counts. |
| QoS and Session Policy | :large_orange_diamond: In progress | Reliable/volatile signaling defaults and retained WebRTC signaling behavior across DDS discovery are in place. | Add workload-specific topic/session profiles and verify repeated reconnect behavior across ROS, WebRTC, and multi-operator sessions. |
...
| 3D Map Transport Compatibility | :large_orange_diamond: In progress | Marker-based global 3D map topics remain aligned across the stack. The bridge now also routes remote-render RGB-D frames and reassembles auxiliary depth/pose payloads, with ROS as the reliable default and WebRTC explicitly experimental. | Make remote-render WebRTC delivery reliable and add bridge-level stress tests for dense-map traffic during multi-operator join/rejoin windows. |
...
| Integration Automation | :large_orange_diamond: In progress | Bridge ships a gtest + Python CTest suite covering HorusLink codecs/protocol/session, golden wire vectors, frame inspection, and remote-render auxiliary payload handling; remaining end-to-end validation uses SDK examples and the Unity runtime. | Add a CI-driven integration matrix across SDK, bridge, remote-render transport, and Unity harness scenarios. |
