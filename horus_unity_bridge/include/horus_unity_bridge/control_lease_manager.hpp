...
public:
  ...
  bool handle_internal_message(
    int client_fd,
    const std::string & topic,
    const std::vector<uint8_t> & serialized_payload);

  bool authorize_command_publish(
    int client_fd,
    const std::string & topic,
...
private:
  ...
  std::unordered_map<DeniedRateLimitKey, uint64_t,
    DeniedRateLimitKeyHash> denied_publish_event_last_ms_;
  ...
  LeaseRequestResult apply_acquire_request_locked(
    int client_fd,
    ...
    uint64_t now_ms);
  LeaseRequestResult apply_heartbeat_request_locked(
    int client_fd,
    const std::string & request_id,
    const std::string & robot_name,
    const std::string & app_id,
    ...
    const std::string & task_kind,
    uint64_t now_ms);
  LeaseRequestResult apply_release_request_locked(
    int client_fd,
    const std::string & request_id,
    const std::string & robot_name,
    uint64_t now_ms);
  ...
  bool should_publish_denied_event_locked(
    int client_fd, const std::string & topic,
    uint64_t now_ms);

  static uint64_t now_ms();
  static bool lease_usage_active(const RobotControlLease & lease);
  static bool try_decode_string_msg(
    const std::vector<uint8_t> & serialized_payload,
    std::string & out_data);
};
