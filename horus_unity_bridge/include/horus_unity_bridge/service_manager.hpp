...
// Generic service server support remains explicitly typed below; the bridge
// uses Jazzy GenericClient only for Unity -> ROS service clients.
 ...
 *
 * Features:
 * - Generic async ROS service calls (Unity -> ROS) for ROS 2 Jazzy GenericClient-compatible service types
 * - Unity service dispatch for explicitly supported service types (Trigger, SetBool, AddTwoInts)
...
public:
   ...
   */
  void handle_unity_service_response(
    uint32_t srv_id,
    const std::vector<uint8_t> & response);
...
private:
  ...
  struct GenericServiceClientWrapper
  {
    ...
    explicit GenericServiceClientWrapper(std::shared_ptr<rclcpp::GenericClient> c)
    : client(c) {}
  };
  ...
  struct TypedUnityServiceServer : public UnityServiceServerBase
  {
    ...
    TypedUnityServiceServer(
      typename rclcpp::Service<ServiceT>::SharedPtr s,
